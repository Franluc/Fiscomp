...
varianzamsd=np.zeros(len(timestep))

g=open('difusion_T_4.0_msd_N_true.txt','w')
g.write('time-step   Msd  VarMsd  SigmaMsd  Diffusion \n')


for tdelay in range(1,len(timestep)-1000,100): #los primeros 50 steps 150=200-50
    suma=0
    sumacuad=0
    ...
    print (msd[tdelay])
    varianzamsd[tdelay]=((((sumacuad)/((Numpart*(len(timestep)-tdelay))-1))-(Numpart*(len(timestep)-tdelay)))*((msd[tdelay])**2)/((Numpart*(len(timestep)-tdelay))-1))/((Numpart*(len(timestep)-tdelay))**2)
    print('+-')
    print(np.sqrt(abs(varianzamsd[tdelay])))
    
    g.write(str(tdelay)+' '+str(msd[tdelay])+' '+str(varianzamsd[tdelay])+' '+str(np.sqrt(abs(varianzamsd[tdelay])))+' '+str(float(msd[tdelay])/float((6*tdelay)))+'\n')
